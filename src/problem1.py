...
def run_test_problem1a():
    ...
    # Test 4:
    expected = -0.846  # This is APPROXIMATELY the correct answer.
    print_expected_result_of_test([10, 200], expected, test_results,
                                  format_string)
    actual = problem1a(100, 200)
    print_actual_result_of_test(expected, actual, test_results, precision=3)
...
def problem1c(n):
    ...
    for k in range(n + 1):
        if k >= 2:
            count = count + sum_of_digits(is_prime(k))
            print(count)
    return count
