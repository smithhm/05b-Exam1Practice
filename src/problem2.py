...
def problem2b(rect, n, delta, win):
    ...
    rect.attach_to(win)
    for k in range(n-1):
        rect_other = rg.Rectangle(rg.Point(rect.corner_1.x + (2*delta*(k+1)), rect.corner_1.y + (2*delta*(k+1))),
                                  rect.corner_2)
        rect_other.move_by(-delta*(k+1), -delta*(k+1))
        rect_other.attach_to(win)
    win.render()
